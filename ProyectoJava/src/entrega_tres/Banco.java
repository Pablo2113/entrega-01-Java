...
// Empleado más joven
	public Optional<Persona> empleadoMasJoven() {
	     return this.empleados.todos().stream()
	                ...
	                .min(Comparator.comparing(Persona::fechaDeNacimiento));
	    }
	
// Número de cuentas de cada cliente
	public Map<String, Integer> numeroDeCuentasDeCliente() {
		 Map<String, Integer> cuentaPorCliente = new HashMap<>();
		    for (Prestamo prestamo : prestamos.todos()) {
		        String dni = prestamo.dniCliente();
		        cuentaPorCliente.put(dni, cuentaPorCliente.getOrDefault(dni, 0) + 1);
		    }
		    return 	cuentaPorCliente;
	}
	...
	public static void main(String[] args) {
		...
		//Preuba prestamo deEmpleado
		System.out.println(banco.prestamosDeEmpleado("86921364V"));
		
		
		//Prueba prestamo de cliente
		System.out.println(banco.prestamosDeCliente("62822051N"));
		
		
		//Prueba cliente mas joven
		
		System.out.println(banco.empleadoMasJoven());
		
		//Prueba numero cuentas cada cliente
